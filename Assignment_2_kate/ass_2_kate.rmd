...
```
```{r}
highschool_att$Threshold <- threshold
threshold
```
```{r}
...
IC_greedy <- function(network, Pprob, seed_node) {
  ...
  max_infected <- 122 # initialize maximum number of infected nodes
...
```
```{r}
greedy_nodes <- c(101, 29, 11)
```
```{r}
plot(IC_average_d(107, Highschool, 0.15,immuned_nodes_betweenness), type = "l", col = "red", ylim = c(0, 20), xlab = "Day", ylab = "Average number of infected people")
lines(IC_average_d(107, Highschool, 0.15, immuned_nodes_degree), type = "l", col = "blue")
lines(IC_average_d(107, Highschool, 0.15, greedy_nodes), type = "l", col = "green")
legend("topright", c("Highschool_betweenness", "Highschool_degree", "white"), col = c("red", "blue", "green"), lty = 1)
```
```{r}
IC_average <- function(node_seed,network,Pprob){
  n <- 100
