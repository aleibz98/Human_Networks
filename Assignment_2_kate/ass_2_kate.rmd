...
```
```{r}
#make a new network with only female n
```
...
for (i in 1:n_obs) {
  idx <- which(u[i] <= cdf)[1]
  thresholds[i] <- dist$Threshold[idx]
}
...
```
```{r}
th1 <- ThModel(c(59,63,91,92,99),Highschool,0.15)
th1
```
```{r}
