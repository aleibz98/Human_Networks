...
```
```{r}
SIR_average(5, Highschool, 0.15)
```
```{r}
SIR_average(5, Highschool2, 0.15)
```
```{r}
SIR_average(5, Highschool3, 0.15)
```
```{r}
plot(SIR_average(5, Highschool, 0.8), type = "l", col = "red", ylim = c(0, 36), xlab = "Day", ylab = "Average number of infected people")
lines(SIR_average(5, Highschool2, 0.8), type = "l", col = "blue")
lines(SIR_average(5, Highschool3, 0.8), type = "l", col = "green")
legend("topright", c("Highschool", "Highschool2", "Highschool3"), col = c("red", "blue", "green"), lty = 1)
```
```{r}
plot(SIR_average(5, Highschool, 0.08), type = "l", col = "red", ylim = c(0, 36), xlab = "Day", ylab = "Average number of infected people")
lines(SIR_average(5, Highschool2, 0.08), type = "l", col = "blue")
lines(SIR_average(5, Highschool3, 0.08), type = "l", col = "green")
legend("topright", c("Highschool", "Highschool2", "Highschool3"), col = c("red", "blue", "green"), lty = 1)
```
```{r}
plot(SIR_average(5, Highschool, 0.5), type = "l", col = "red", ylim = c(0, 36), xlab = "Day", ylab = "Average number of infected people")
lines(SIR_average(5, Highschool2, 0.5), type = "l", col = "blue")
lines(SIR_average(5, Highschool3, 0.5), type = "l", col = "green")
legend("topright", c("Highschool", "Highschool2", "Highschool3"), col = c("red", "blue", "green"), lty = 1)
```
```{r}
plot(SIR_average(5, Highschool, 0.8), type = "l", col = "red", ylim = c(0, 36), xlab = "Day", ylab = "Average number of infected people")
...
threshold <- c(0.4, 0.47, 0.49, 0.72, 075)


```
```{r}
ThModel(values, Highschool, threshold)

```
```{r}
highschool_att$Threshold
```
```{r}
```
```{r}
...
IC(107, Highschool, 0.15, immuned_nodes_degree)
```
```{r}
immuned_nodes_betweenness <- c(4,37,96)
```
```{r}
IC(107, Highschool, 0.15, immuned_nodes_betweenness)
...
lines(IC(5, Highschool, 0.15, immuned_nodes_degree), type = "l", col = "blue")
...
```
```{r}
IC_greedy<-function(node_seed,network,Pprob){
...
```{r}
IC_greedy <- function(network, Pprob, num_immunized = 3) {
  nNode <- vcount(network)
  node_status <- rep.int(0, nNode) #start from a healthy population
  node_status[107] <- 1 # node 107 is infected on day 1
  day_infected <- vector() # Total number of infected population
  new_infected <- list()  # Record the ID of person getting infected at each time step
  immunized <- c() # List of immunized nodes
  ...
  day <- 1
  day_infected[day] <- sum(node_status)
  new_infected[[day]] <- 107 # The ID of the person infected in Day 1 (Patient Zero)

  # Simulate the spread of virus within 4 weeks
  for (day in 2:28) {
    # Find the most influential node to immunize
    max_reduction <- 0
    node_to_immunize <- NULL
    for (i in 1:nNode) {
      if (i %in% immunized) next # skip already immunized nodes
      if (node_status[i] == 0) next # skip already infected nodes
      tmp_status <- node_status # create a temporary copy of the current node status
      tmp_status[i] <- 2 # immunize the current node
      tmp_infected <- unlist(lapply(which(tmp_status == 1), calculate_value, each_neighbors, Pprob))
      reduction <- sum(!tmp_infected %in% new_infected[[day-1]])
      if (reduction > max_reduction) {
        max_reduction <- reduction
        node_to_immunize <- i
      }
    }
    print(node_to_immunize)
    # Immunize the node with the highest reduction
    node_status[node_to_immunize] <- 2
    immunized <- c(immunized, node_to_immunize)
    if (length(immunized) == num_immunized) break # stop immunizing nodes when reaching the desired number

    # Simulate the spread of the virus for the current day
    contagious_ID <- which(node_status == 1)
    infected_ID <- unlist(lapply(contagious_ID, calculate_value, each_neighbors, Pprob))
    new_infected_ID <- setdiff(infected_ID, which(node_status == 1))

    # Update the node status and other variables
    node_status[new_infected_ID] <- 1
    day_infected[day] <- length(new_infected_ID)
    new_infected[[day]] <- new_infected_ID
  }
  return(day_infected)
  # return(list(day_infected, immunized)) # return the number of newly infected people and the list of immunized nodes by day
}

```
```{r}
#
IC_greedy(Highschool, 0.15, 3)
```
```{r}
IC_greedy_med <- function(network, Pprob, num_immunized = 3) {
  n <- 50
  result <- matrix(0, nrow = 28, ncol = n)
  for (i in 1:n){
    result[, i] <- IC_greedy(network, Pprob, num_immunized = 3)
  }
  return(apply(result, 1, median))
}
```
```{r}
IC_greedy_med(Highschool, 0.15, 3)
```
```{r}
```
