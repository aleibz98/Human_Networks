...
```
```{r}
#make a plot with how the average path length changes with the rewiring probability in Regular8
p<-seq(0,1,0.01)
mean_distance<-c()
for (i in p){
...
# plot(p,mean_distance,type="l",xlab="Rewiring probability",ylab="Average path length")
#apply log scale to the mean_distance and plot it again
plot(p,mean_distance,type="l",xlab="Rewiring probability",ylab="Average path length")
```
```{r}
...
```
```{r}
#make a plot with how the clustering coefficient changes with the rewiring probability in Regular8
p<-seq(0,1,0.01)
mean_distance<-c()
for (i in p){
  Regular8<-watts.strogatz.game(dim=1,size=300,nei=6, p=i)
  mean_distance<-c(mean_distance,transitivity(Regular8))
}
plot(p,mean_distance,type="l",xlab="Rewiring probability",ylab="Clustering coefficient")

```

```{r}
#combine two previous lines of code into one
p<-seq(0,1,0.01)
mean_distance<-c()
clustering<-c()
for (i in p){
...
plot(p,log(mean_distance),type="l",xlab="Rewiring probability",ylab="Average path length")
plot(p,log(clustering),col="red")
...
plot(p,log(mean_distance),type="l",xlab="Rewiring probability",ylab="Average path length",ylim=c(0,1))
plot(p,log(clustering),col="red")
legend("topright",c("Average path length","Clustering coefficient"),col=c("black","red"),lty=1)

```
```{r}
g0 <- barabasi.game(100, power = 0.2, m = NULL, out.dist = NULL, out.seq = NULL, out.pref = FALSE, zero.appeal = 1, directed = FALSE,algorithm ="psumtree", start.graph = NULL)
...
# Create a graph
G <- graph(edges = c(1, 2, 2, 3, 3, 4, 4, 1, 1, 3))

# Calculate the current average path length
apl_current <- mean(shortest.paths(Highschool))

# Loop until a significant increase in average path length is achieved
while (TRUE) {
  # Select a random edge
  edges <- E(Highschool)
  if (length(edges) == 0) break  # No edges left to remove
  edge_to_remove <- sample(edges, 1)

  # Remove the selected edge and calculate the new average path length
  G_new <- delete.edges(Highschool, edge_to_remove)
  apl_new <- mean(shortest.paths(G_new))

  # Check if the increase in average path length is significant
  increase <- (apl_new - apl_current) / apl_current
  if (increase < 0.05) break  # Increase is not significant

  # Update the current average path length and continue the loop
  G <- G_new
  apl_current <- apl_new
}

# Print the resulting graph and average path length
cat("Resulting graph:", "\n")
plot(G)
cat("Average path length:", apl_current, "\n")
