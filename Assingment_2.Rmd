...
ggplot(data.frame(p), aes(x = p)) +
  geom_line(aes(y = sapply(p, function(x) {
    y <- watts.strogatz.game(dim=1,size=100,nei=6, p=x)
    transitivity(y)/transitivity(z)
  })), color = "red") +
  geom_line(aes(y = sapply(p, function(x) {
    y <- watts.strogatz.game(dim=1,size=100,nei=6, p=x)
    mean_distance(y)/mean_distance(z)
  })), color = "blue") +
  labs(x = "p", y = "Clustering coefficient and Average path length") +
  scale_x_continuous(breaks = seq(0, 1, 0.1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.1)) +
  theme_bw()
  # TODO: Graph is not looking as it should
```
...
# a) the one consisted of Node 55, 107, 93, 109, 80, 28;
ThModel(c(55, 107, 93, 109, 80, 28),Highschool,0.15)

# b) the one consisted of Node 110, 39, 10, 1, 50, 106.
ThModel(c(110, 39, 10, 1, 50, 106),Highschool,0.15)
```
...
IC2 <- function(g, p, t){
  ...
  for(i in 2:t){
    ...
    # Print the number of newly infected nodes at each day
    cat("Day ", i, ": ", sum(state == 1), " new infections\n", sep = "")

    # Save the new infections to a list
    new_infections <- c(new_infections, sum(state == 1))
  }
  # Return the state of each node and the new infections list
  return(list(state, new_infections))
...
# Determine which are the best nodes to immunize

# Degree Heuristics
degree_heuristics <- function(g, p, t, k){
  # Run the IC model
  result <- IC2(g, p, t)
  # Get the state of each node
  state <- result[[1]]
  # Get the new infections list
  new_infections <- result[[2]]
  # Get the degree of each node
  degree <- degree(g)
  # Get the nodes with the highest degree
  highest_degree <- order(degree, decreasing = TRUE)[1:k]
  # Immunize the nodes with the highest degree
  state[highest_degree] <- 2
  # Run the IC model again
  result <- IC2(g, p, t)
  # Get the state of each node
  state <- result[[1]]
  # Get the new infections list
...
betweenness_heuristics <- function(g, p, t, k){
  # Run the IC model
  result <- IC2(g, p, t)
  # Get the state of each node
  state <- result[[1]]
  ...
  # Get the nodes with the highest betweenness
  highest_betweenness <- order(betweenness, decreasing = TRUE)[1:k]
  # Immunize the nodes with the highest betweenness
  state[highest_betweenness] <- 2
  # Run the IC model again
  result <- IC2(g, p, t)
  # Get the state of each node
  state <- result[[1]]
...
# Run the degree heuristics
degree_heuristics(Highschool, 0.15, 10, 3)
# Run the betweenness heuristics
```
